...
class Controller:
	...
	def __init__(self, port="/dev/ttyUSB0"):
		...
		self.client = ModbusClient(port=self._port, baudrate=115200, timeout=1)
		...
		##############
		## 5 inches wheel
		self.travel_in_one_rev = 0.3989832
		self.cpr = 16385
		self.R_Wheel = 0.0635 #meter
	...
	def set_mode(self, MODE):
		...
		result = self.client.write_register(self.OPR_MODE, MODE, self.ID)
		return result

	def get_mode(self):

		# result = self.client.read_holding_registers(self.OPR_MODE, 1, self.ID)
	...
	def enable_motor(self):
		result = self.client.write_register(self.CONTROL_REG, self.ENABLE, self.ID)

	def disable_motor(self):
		result = self.client.write_register(self.CONTROL_REG, self.DOWN_TIME, self.ID)

	def get_fault_code(self):

		fault_codes = self.client.read_holding_registers(self.L_FAULT, 2, self.ID)
	...
	def clear_alarm(self):
		result = self.client.write_register(self.CONTROL_REG, self.ALRM_CLR, self.ID)
	...
	def set_accel_time(self, L_ms, R_ms):
		...
		result = self.client.write_registers(self.L_ACL_TIME, [int(L_ms),int(R_ms)], self.ID)
	...
	def set_decel_time(self, L_ms, R_ms):
		...
		result = self.client.write_registers(self.L_DCL_TIME, [int(L_ms), int(R_ms)], self.ID)
	...
	def set_rpm(self, L_rpm, R_rpm):
		...
		result = self.client.write_registers(self.L_CMD_RPM, [left_bytes, right_bytes], self.ID)
		return result
	def get_rpm(self):


		# rpms = self.client.read_holding_registers(self.L_FB_RPM, 2, self.ID)
	...
	def set_maxRPM_pos(self, max_L_rpm, max_R_rpm):
		...
		result = self.client.write_registers(self.L_MAX_RPM_POS, [int(max_L_rpm), int(max_R_rpm)], self.ID)

	def set_position_async_control(self):

		result = self.client.write_register(self.POS_CONTROL_TYPE, self.ASYNC, self.ID)

	def move_left_wheel(self):

		result = self.client.write_register(self.CONTROL_REG, self.POS_L_START, self.ID)

	def move_right_wheel(self):

		result = self.client.write_register(self.CONTROL_REG, self.POS_R_START, self.ID)
	...
	def set_relative_angle(self, ang_L, ang_R):
		...
		result = self.client.write_registers(self.L_CMD_REL_POS_HI, all_cmds_array, self.ID)
	...
	def get_wheels_travelled(self):
		...
		# 	result = self.client.read_holding_registers(self.L_FB_POS_HI, 4, self.ID)
		# 	try:
